...
function useSelectSearchContext() {
    return React.useContext<SelectSearchContextValue | null>(
        SelectSearchContext
    );
}
...
// 👇 Add searchable + searchPlaceholder props here
...
function SelectContent({
    ...
    children,
    position = "popper",
    searchable = true, // default: all selects are searchable
            ...
            <SelectPrimitive.Portal>
                ...
                >
                    ...
                    {searchable && (
                        <div className="p-1">
                            <input
                                className="flex h-8 w-full rounded-md border border-input bg-background px-2 text-xs outline-none ring-offset-background placeholder:text-muted-foreground focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-1"
                                placeholder={searchPlaceholder}
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                // prevent keys from interfering with Radix keyboard nav
...
function SelectItem({
    ...
    const searchTerm = searchCtx?.searchTerm ?? "";

    // Decide what text to match for searching
    const labelToSearch =
        typeof children === "string" ? children : searchValue ?? "";

    const shouldHide =
        searchTerm &&
        labelToSearch &&
        !labelToSearch.toLowerCase().includes(searchTerm.toLowerCase());
